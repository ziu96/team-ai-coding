...
在 Claude Code 中按顺序完成。**Add Marketplace 的单行输入框只填写一个来源，不能把后续命令一起粘进去。**

1. 输入 `/plugin marketplace add`；在弹出的 **Enter marketplace source** 中只填：
...
建议选择 `user`，以便在多个业务仓库复用；仅做单仓库试验时才选 `project`。
...
├── rules/                          # Cursor 安装即载入的 L0/L1/L2 路由
