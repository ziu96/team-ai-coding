...
- 恢复 init 完成后的可选项目上下文补全：核心模板验收后，明确询问是否根据仓库已有事实补全 `openspec/project.md` 与 `openspec/roadmap.md`；先展示来源与拟写内容，第二次确认后才写入。
- 保留固定模板和不覆盖边界：上下文补全仅适用于本次新建、仍与模板一致的两份文件；既有文件继续只读保留。
- 补全 Cursor 的个人/项目安装、范围选择、验证、初始化和更新步骤。
